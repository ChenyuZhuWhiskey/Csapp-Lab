...
* 
* Brief introduction: 
*       This solution used an algorithm with segregated mm_free list 
*   and a binary search tree combined to maximize both space utilization 
*   and throughput. Please note that each mm_free list is a unique size class, 
*   which means it only contains blocks with EXACTLY THE SAME SIZE. 
*  
* Details of binary search tree: 
*       The BST is used to store mm_free blocks sized larger than a specific 
*   threshold (I set it to 40 bytes), of which each node is a header of a 
*   mm_free list of a same size, When searching for a mm_free block in BST, 
*   I applied the best-fit policy which selects the mm_free block with 
*   approximately minimum size among all the blocks sized larger than  
*   requested. 
* 
* Details of segregated mm_free list: 
*       I set up a small array to store the headers of segregated mm_free lists. 
*   Each mm_free list, as described above, has blocks with a same size. Blocks 
...
static block_ptr *bins;             /* Array of the headers of segregated mm_free lists */  
static const unsigned int fixed_bin_count = 5;  /* Number of segregated mm_free lists */  
#ifdef DEBUG  
static int operid;  
#endif  
...
/* Reset the fields of a mm_free block bp */  
...
/* Remove bp from its mm_free list */  
...
{  
    ...
    {  
        ...
        /* Remove a mm_free list header from the array of headers */  
...
{  
    ...
    /* Search the mm_free list for a fit */  
...
{  
    ...
    /* If size == 0 then this is just mm_free, and we return NULL. */  
    if (size == 0)  
    {  
        mm_free(oldptr);  
        return 0;  
    ...
    /* If oldptr is NULL, then this is just mm_malloc. */  
    if (oldptr == NULL)  
    {  
...
/* 
 * mm_calloc - I don't want to look at mm-naive.c 
 * This function is not tested by mdriver, but it is 
 * needed to run the traces. 
 */  
block_ptr mm_calloc(size_t nmemb, size_t size)  
{  
...
/* 
 * extend_heap - Extend heap with mm_free block and return its block pointer 
 */  
static block_ptr extend_heap(size_t words)  
{  
    char *bp;  
    ...
    /* Initialize mm_free block header/footer and the epilogue header */  
    PUT(HDRP(bp), PACK(size, 0));           /* Free block header */  
    PUT(FTRP(bp), PACK(size, 0));           /* Free block footer */  
    reset_block(bp);  
    PUT(HDRP(NEXT_BLKP(bp)), PACK(0, 1));   /* New epilogue header */  
  
    /* Coalesce if the previous block was mm_free */  
    return coalesce(bp);  
}  
  
/* 
 * place - Place block of asize bytes at start of mm_free block bp 
...
/* 
 * insert_free_block - insert a block into BST or segregated mm_free list 
...
{  
    ...
    {  
        /* Insert into segregated mm_free list */  
...
{  
    ...
    {  
        if (bins[dcount - 1])  
        {  
            /* Found a mm_free list of this size! */  
            curr = bins[dcount - 1];  
